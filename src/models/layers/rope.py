...
class RotaryPositionEmbedding2D(nn.Module):
    ...
    def _apply_1d_rope(
        ...
        """
        # Ensure positions are valid integers and within bounds
        positions = positions.long()  # Convert to long integers
        positions = torch.clamp(positions, 0, cos_comp.size(0) - 1)  # Clamp to valid range
...
class RotaryPositionEmbedding4D(RotaryPositionEmbedding2D):
    """4D Rotary Position Embedding implementation.

    This module extends the 2D Rotary Position Embedding to handle 4D spatial positions.
    It applies rotary position embeddings to input tokens based on their 4D spatial positions.
    ...
    def forward(self, tokens: torch.Tensor, positions: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            tokens: Input tensor of shape (batch_size, n_heads, n_tokens, dim).
                   The feature dimension (dim) must be divisible by 8.
            positions: Position tensor of shape (batch_size, n_tokens, 4) containing
                      the w, z, y, and x coordinates for each token.
        ...
        # Split features for w, z, y, and x processing
        w_features, z_features, y_features, x_features = tokens.chunk(4, dim=-1)

        # Apply RoPE separately for each dimension
        w_features = self._apply_1d_rope(w_features, positions[..., 0], cos_comp, sin_comp)
        z_features = self._apply_1d_rope(z_features, positions[..., 1], cos_comp, sin_comp)
        y_features = self._apply_1d_rope(y_features, positions[..., 2], cos_comp, sin_comp)
        x_features = self._apply_1d_rope(x_features, positions[..., 3], cos_comp, sin_comp)

        # Combine processed features
        return torch.cat((w_features, z_features, y_features, x_features), dim=-1)
