...
class TripoSGEncoderCS(nn.Module):
    """V1 has cross-attention and self-attention interleaved.
    """
    def __init__(
        self,
        in_channels: int = 3,
        ...
        num_attention_heads: int = 8,
        num_layers: int = 6,
        num_cross_attention_per_layer: int = 1,
        num_self_attention_per_layer: int = 1,
        **kwargs,
    ):
        super().__init__()
        ...
        for _ in range(num_layers):
            # Add cross attention blocks
            for _ in range(num_cross_attention_per_layer):
                self.blocks.append(
                    DiTBlock(
                        dim=dim,
                        num_attention_heads=num_attention_heads,
            ...
            # Add self attention blocks
            for _ in range(num_self_attention_per_layer):
                self.blocks.append(
                    DiTBlock(
                        dim=dim,
                        num_attention_heads=num_attention_heads,
        ...
        self.num_cross_attention_per_layer = num_cross_attention_per_layer
        self.num_self_attention_per_layer = num_self_attention_per_layer

    def forward(self, sample_1: torch.Tensor, sample_2: torch.Tensor):
        hidden_states = self.proj_in(sample_1)
        encoder_hidden_states = self.proj_in(sample_2)

        blocks_per_layer = self.num_cross_attention_per_layer + self.num_self_attention_per_layer
        
        for i, block in enumerate(self.blocks):
            # Determine block type based on position within layer
            position_in_layer = i % blocks_per_layer
            
            if position_in_layer < self.num_cross_attention_per_layer:  # Cross attention
                hidden_states = block(
                    hidden_states, encoder_hidden_states=encoder_hidden_states
                )
            else:  # Self attention
...
class TripoSGVAEModel(ModelMixin, ConfigMixin):
    ...
    def _encode(
        ...
        # positions.shape [B, N, 3]
        # features.shape [B, N, 3] 
        positions, features = x[..., :position_channels], x[..., position_channels:]

        # x_kv.shape= [B, N, position_channels + embedder.out_dim]
    ...
    ) -> Union[AutoencoderKLOutput, Tuple[torch.Tensor]]:
        """
